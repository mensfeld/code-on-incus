...
### 1. False Positive: Normal Build Operations
**Test**: `test_normal_build_operations_no_alert`
**Issue**: Benign build script (`echo`, `ls`, `cat`) triggers container kill
**Status**: Needs investigation - possibly background Docker processes triggering detection
**Priority**: Medium - core functionality works, this is an edge case

### 2. PHP Reverse Shell Detection  
**Test**: `test_php_reverse_shell_detection`
**Issue**: Pattern `php -r fsockopen` not being detected
**Expected**: Should match "php -r" + "fsockopen" patterns and isNetworkRelated check
**Status**: Possible timing issue or pattern matching bug
**Priority**: Low - Python and Perl equivalents work

### 3-5. Filesystem Monitoring (3 tests)
**Tests**: 
