...
- **Host-side auth.log / syslog monitoring** — The monitoring daemon now tails the container's `auth.log` and `syslog` from the host by reading `/proc/<init-pid>/root/var/log/<file>` directly, polling every 5 seconds and advancing a byte offset so only new lines are processed. Because the read happens outside the container, an attacker inside cannot suppress alerts by manipulating log files, replacing `syslog`, or killing the logging daemon. When the host-side path is unreadable (e.g. the monitor process lacks ptrace permission in the container's user-namespace), the watcher falls back transparently to `incus file pull` via the Incus daemon so detection continues in all deployment configurations. The watcher detects failed SSH logins, invalid users, PAM authentication failures, unauthorized sudo attempts, `su` privilege escalation failures, and unexpected root sessions. Failed logins and root sessions raise `WARNING` threats; sudo/su privilege escalation attempts raise `HIGH` threats. The watcher silently skips containers whose images do not ship auth.log or syslog (minimal/distroless containers) and automatically picks up the files if they are created after daemon startup.
