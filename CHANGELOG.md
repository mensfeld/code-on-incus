...
- [Bug Fix] **Silent skip of non-existent protected paths allowed host persistence attack** (P1-1, FLAWS Finding 2) — `internal/session/security.go` only auto-created `.git/hooks` and silently skipped every other missing entry in `security.protected_paths` (`.vscode`, `.husky`, user-added paths). The read-only overlay was never established, so a process inside the container could `mkdir /workspace/.vscode && echo pwn > /workspace/.vscode/tasks.json`; the write landed on the writable `/workspace` bind mount and persisted to the host, enabling a tasks.json auto-execute attack the next time the host user opened the repo. Same attack shape worked for `.husky/pre-commit` and any additional protected path. Fixed by materializing every protected entry before mounting: directory-type entries are created with `MkdirAll`, and the single file-type entry `.git/config` gets an empty placeholder only when its parent `.git/` directory already exists (non-git workspaces remain untouched). Existing files and directories are never clobbered. Finding 1 (mountns escape via `sudo unshare -m`) remains open and is tracked separately. (#310)
