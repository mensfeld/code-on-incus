...
- **macOS: hint how to bring a Keychain-stored login into the container (#818)** — the companion to #817. On macOS Claude Code keeps its OAuth token in the login **Keychain** (item `Claude Code-credentials`), not in `~/.claude/.credentials.json`, and coi — running inside the Linux VM — cannot read the Keychain, so there may be no credential file to seed even after #817 resolves the shared-home lookup. When coi detects this (a macOS VM, a tool whose credential is Keychain-backed, and no credential file at the resolved config dir) it now prints a one-shot hint with the exact `security find-generic-password -s "Claude Code-credentials" -w > ~/.claude/.credentials.json` command to run **on the Mac**, plus the API-key alternative. The macOS Setup Guide gains a "Claude auth on macOS" section documenting both. (Keychain-backed credentials are declared per tool via the credential catalog, so this generalizes beyond Claude.)
