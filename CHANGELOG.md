...
- **Headless, tool-agnostic launch: `coi shell --headless` (#746)** — a non-interactive entry point so an external orchestrator can drive **any** coi-supported tool without re-implementing its CLI. coi builds the command from the profile's tool via the existing `tool.Tool` abstraction (session id, resume, model, permission, and the profile's tool env from #744); the caller supplies only the dynamics: `--prompt-file` (initial prompt), `--system-prompt-file` (for tools that support one, e.g. Claude's `--append-system-prompt`), and `--resume`/`--continue` (continue-or-fresh: resume if a prior session exists, else start fresh). The run launches into the detached tmux session the caller already drives with `coi tmux capture`/`send`, and `--json` prints machine-readable handles (`container`, `tmux_session`, `session_id`, `exit_file`). A new **`coi tmux status <container> --session-id <id>`** reports the run as `running`/`done` from a completion sentinel the launch records, with `--wait` (block until done, exit with the tool's code) and `--json`. Prompt/system-prompt content is staged as in-container files and referenced via `"$(cat …)"`, so arbitrary text (quotes/newlines) can't corrupt the launch. Adding a new tool stays a coi-side `tool.Tool` change with zero orchestrator work. New capability interface `ToolWithPrompt` (implemented by Claude and Codex; opencode's initial prompt is pasted into the pane). Note: the per-tool prompt/system-prompt flags are modeled on each CLI's documented shape and should be validated against the tool versions you run.
