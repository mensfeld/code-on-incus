...
- **`[limits.disk] size` — whole-container disk quota, plus a default `/tmp` cap (#728)** — you can now bound a container's entire root filesystem from a profile: `[limits.disk] size = "20GiB"` sets the Incus root device `size=` quota, applied through the same root-device path as the existing I/O limits (`read`/`write`/`max`). Because Incus only *enforces* a root `size=` quota on quota-capable storage drivers, coi resolves the container's pool driver and **hard-errors** when `size` is set on a `dir` pool (which silently ignores it) — a clear "use a btrfs/zfs/lvm pool" message beats believing you're capped when you aren't. Complementing that, `/tmp` sizing (the `tmpfs_size` mechanism from #733) now applies a **2GiB default cap when `tmpfs_size` is unset**, but only when it's safe to do so opportunistically — `/tmp` isn't already a bounded tmpfs, and nothing is mounted under `/tmp` (a fresh tmpfs would shadow it, e.g. a `preserve_workspace_path` workspace living under `/tmp`). So a runaway build can't exhaust `/tmp` out of the box, while a custom image that deliberately sizes `/tmp` is left untouched. An explicit `tmpfs_size` always wins. `coi profile info` now shows `size`/`tmpfs_size`. Covered by unit tests (size-format validation, pool-driver capability, the `/tmp`-bounded probe, and config merge).
...
- **`/tmp` sizing now applies on `coi run`, not just `coi shell` (#728 follow-up)** — the `[limits.disk] tmpfs_size` cap (and the new 2GiB default) was added inline in `session.Setup`, so it only ran on the shell launch path; `coi run` — which already honors every other profile limit through the shared resource-limit applier — silently skipped `/tmp` sizing. Extracted the logic into a shared `session.ApplyTmpfsSizing` helper called by both the shell (`session.Setup`) and run (`configureContainerRunPhase`, once the container is ready) paths, so a profile's `/tmp` sizing is honored uniformly however the container is launched. Explicit `tmpfs_size` and the opportunistic default (skipped when `/tmp` is already bounded or has submounts) behave identically on both paths. Covered by a unit test on the shared helper.
