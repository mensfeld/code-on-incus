...
- **`coi top` container view now reports the whole container's memory/CPU/I/O, not just its init process** — the per-container row read cgroup counters from the path `GetCgroupPath` returns, which on systemd containers (via the `incus info` fallback) is the init process's own `init.scope` sub-cgroup. `memory.current` there accounts for only PID 1, so every container showed a near-constant ~8 MB while `coi top <name>` (per-process view, which reads true RSS from `/proc`) correctly summed to hundreds of MB. `CollectResourceStats` now strips the trailing systemd scope/service segment and reads the container's top-level cgroup, whose cgroup v2 counters aggregate the entire process tree — the same container-root resolution the process view already used, now shared via a `containerRootCgroupPath` helper. The I/O read's one-level parent climb (added to work around `init.scope` tracking no I/O) is removed, since the root is read directly and climbing further would reach the grouping cgroup shared by all containers and over-count.
