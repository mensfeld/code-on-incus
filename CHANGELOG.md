...
- **Fix sandbox settings injection using pure Go instead of Python** — Sandbox settings (e.g., opencode permission bypass, Claude effort level) are now merged into tool config files using Go's `encoding/json` instead of a Python one-liner executed inside the container. This fixes intermittent "exit status 1" failures when the source JSON file contains non-standard content (comments, trailing commas, BOM) and eliminates the dependency on `python3` being available inside the container. (#351, #355)
