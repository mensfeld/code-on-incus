...
- [Bug Fix] **`make build` crashes with cryptic cgo error when `libsystemd-dev` is missing** — On a fresh Ubuntu/Debian box without `pkg-config` or `libsystemd-dev` installed, building from source failed with an opaque cgo/pkg-config error from the `sdjournal` import in `internal/nftmonitor/journalctl.go`. Added a `check-deps` preflight target to the `Makefile` (now a prerequisite of `build` and `build-all`) that detects missing `pkg-config` or `libsystemd` headers on Linux and prints an actionable message with install commands for apt, dnf, and pacman. The check is a no-op on macOS/Darwin since `sdjournal` is gated by `//go:build linux` and never links `libsystemd` on Darwin. Includes a Python integration test in `tests/meta/check_deps_test.py` that launches a bare Ubuntu container, verifies the friendly error is printed when `pkg-config`/`libsystemd-dev` is missing, and that the check passes once the packages are installed.
