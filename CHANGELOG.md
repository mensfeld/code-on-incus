...
- **Profile-based base image override and ubuntu: remote auto-registration** — `coi build` continues to use `images:ubuntu/22.04` (LXC CDN) as its default base, but users whose network blocks that CDN can now override the base image per-profile. Set `[container.build] base = "ubuntu:jammy"` (or any Incus-compatible image reference) in `~/.coi/config.toml` or `.coi/config.toml` to pull from a different source. When an image reference starts with `ubuntu:`, COI automatically registers the `ubuntu:` simplestreams remote (`cloud-images.ubuntu.com/releases`) if it is not already present in Incus — no manual `incus remote add` needed. This override also applies to the default `coi-default` image, not just custom profiles. (#388)
