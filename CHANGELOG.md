...
- **Error messages no longer suggest the non-existent `--network=open` flag** — When firewalld was unavailable or conflicting with ufw, COI told users to run `coi shell --network=open`, but `--network` was removed as a CLI flag in an earlier refactor. Following the advice produced `Error: unknown flag: --network`. The four affected messages (in `network/manager.go` and `health/checks.go`) now point to the correct config-file approach: set `mode = "open"` in the `[network]` section of your `config.toml`. Closes #398.
