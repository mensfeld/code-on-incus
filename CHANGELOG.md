...
- [Bug Fix] **Enforce /tmp size limit and add auto-cleanup to prevent agents hanging on full disk** - Containers now have a hard /tmp tmpfs size limit applied at startup, preventing coding agents from hanging indefinitely when /tmp fills up. The `SetTmpfsSize()` implementation was corrected to use the `size=` property directly on the Incus disk device (the previous `limits.memory.tmpfs` config key was invalid and never applied). The call was also re-enabled after being disabled with a TODO. Default size increased from 2GiB to 4GiB to give agents more headroom. The size is configurable via `tmpfs_size` under `[limits.disk]` in `config.toml` and now correctly inherits through config merging. The base image now also ships `/etc/tmpfiles.d/coi-tmp-cleanup.conf` (1-hour age threshold) and a `systemd-tmpfiles-clean.timer` override (15-minute interval) so abandoned build artefacts are reclaimed automatically between operations. Includes unit tests for default value and merge behaviour, and integration tests verifying the reported /tmp size matches the configured value. Adds `docs/Troubleshooting-Agent-Hangs.md` explaining root cause, symptoms, and remediation. Fixes #135.
