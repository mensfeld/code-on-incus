...
- **`coi file pull` / session-state save no longer recreate container symlinks that escape the destination on the host** — `PullDirectory` copies container-controlled content to the host (used by `coi file pull` and the automatic session-state save of the agent-writable tool config dir). Symlinks were recreated verbatim, so a container-planted symlink like `x -> /home/user/.ssh/authorized_keys` or `../../../../etc/shadow` would be materialized on the host — a symlink-extraction (Zip-Slip-class) host-tampering vector. `PullDirectory` now sanitizes the pulled tree before materializing it (`sanitizePulledSymlinks`): any symlink whose target resolves outside the pulled tree (absolute paths or `..` traversal) is dropped with a warning, while intra-tree relative links and regular files are preserved. Applied at the pull boundary so it covers both the `os.Rename` fast path and the cross-device `copyDirRecursive` fallback. Defense-in-depth host protection.
