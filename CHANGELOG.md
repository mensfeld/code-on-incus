...
- **`coi overview` — a live table of running sessions (#341, #359)** — a read-only command that shows each running coi session, the container backing it, the workspace it was launched against, its mode (ephemeral/persistent), start time, and uptime, refreshing every `--refresh-interval` seconds (default 2; Ctrl+C to exit). When stdout is **not** a terminal it renders a single snapshot and returns instead of looping, so it stays usable from cron, scripts and CI — TTY detection uses the `TIOCGWINSZ` ioctl (not `os.ModeCharDevice`, which would misreport a `/dev/null` redirect as a TTY and trap the command in its refresh loop). No container actions are taken from the command.
