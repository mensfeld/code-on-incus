...
- **Runtime-loadable exec pattern database** — The PROC_EVENTS watcher now loads suspicious-exec patterns from `~/.coi/exec-patterns.toml` at daemon startup, enabling pattern updates without releasing a new `coi` version. The compiled-in fallback set has been extended with eight additional reverse-shell patterns derived from GTFOBins: PHP `fsockopen`, Ruby `-rsocket`, Perl `sockaddr_in`, Node.js `child_process`+`net`, Lua lua-socket, gawk `/inet/tcp/`, zsh `ztcp`, and busybox `nc -e`. The new `coi update-patterns` command clones the GTFOBins database to `~/.coi/gtfobins/` and regenerates the patterns file — run it to pick up new reverse-shell techniques without upgrading `coi`.
...
- **Host-side auth.log / syslog monitoring** — The monitoring daemon now tails the container's `auth.log` and `syslog` from the host via `/proc/<init-pid>/root/var/log/` using inotify. Because the read happens outside the container, an attacker inside cannot suppress alerts by manipulating log files, replacing `syslog`, or killing the logging daemon. The watcher detects failed SSH logins, invalid users, PAM authentication failures, unauthorized sudo attempts, `su` privilege escalation failures, and unexpected root sessions. Failed logins and root sessions raise `WARNING` threats; sudo/su privilege escalation attempts raise `HIGH` threats. The watcher silently skips containers whose images do not ship auth.log or syslog (minimal/distroless containers) and automatically picks up the files if they are created after daemon startup.
