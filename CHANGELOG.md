...
- **Switch base image source from LXC CDN to Canonical's official Ubuntu remote** — `coi build` previously pulled `images:ubuntu/22.04` from the LXC community image server, which is distributed via a third-party CDN (mirrors like `sgp1lxdmirror01.do.letsbuildthe.cloud`). This CDN is blocked in some corporate networks. The base image is now `ubuntu:22.04` from Canonical's own `cloud-images.ubuntu.com` infrastructure. COI automatically registers the `ubuntu:` simplestreams remote in Incus if it is not already present, so no manual setup is required. The resulting `coi-default` container image is functionally identical — only the download source changes. **If you are upgrading:** run `coi build --force` to rebuild from the new source. To override the base image (e.g. back to `images:ubuntu/22.04` or a local mirror), set `[container.build] base = "..."` in your `~/.coi/config.toml` or project `.coi/config.toml`. (#388)
