...
- **Project (`.coi/config.toml`) config is treated as untrusted: mounts are confined to the workspace and security-weakening network flags are refused** — the workspace's `.coi/config.toml` (which a cloned repo can ship, or an agent can plant) was trusted as much as the user's own `~/.coi/config.toml`. A project config could `[[mounts.default]]` an arbitrary host path (`~`, `~/.ssh`, `/etc`) writable into the container (→ host code execution via UID-shifted writes), and could silently set `block_private_networks=false` / `block_metadata_endpoint=false` / `mode=open` (→ cloud metadata/SSRF and private-network exposure). Config is now loaded with a trust scope: `~/.coi/config.toml` and an explicit `COI_CONFIG` are trusted; the project `.coi/config.toml` is untrusted. Untrusted mounts are tagged `FromProject` and must resolve **inside the workspace** (`ParseMountConfig` rejects escapes); untrusted network downgrades are dropped with a warning, while strengthening values are still honored. User/`COI_CONFIG` mounts remain unrestricted.
