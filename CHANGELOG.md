...
- [Feature] **opencode support** - Added opencode (https://opencode.ai) as a supported AI coding tool. opencode is installed in the default base image alongside Claude CLI. Configure it with `[tool] name = "opencode"` in `.coi.toml`. COI automatically copies `~/.opencode.json` from the host (if present) into the container and merges the permission bypass config (`"permission": {"*": "allow"}`) so opencode runs without interactive permission prompts. API keys are read from the host `~/.opencode.json` providers section or standard environment variables (`ANTHROPIC_API_KEY`, etc.). Resume support uses `--continue` to resume the last SQLite session. Implements the new `ToolWithHomeConfigFile` optional interface for tools that use a single home-dir JSON file rather than a config subdirectory. Includes 9 unit tests and 2 integration tests that launch the real claude and opencode binaries (no stub) to verify session start and config injection end-to-end. Fixes #117.
