...
- **Per-invocation tool override: `--tool <name>` (#708)** — keep your default in `[tool] name` but launch a different tool for a single run without editing config or making a profile. `coi shell --tool codex` starts that session in codex; `coi attach --tool codex` starts the tool in an already-running container (the analog of `attach --bash`, seeding that tool's config/creds/context first so e.g. codex authenticates on a claude-created container); `coi run --prompt … --tool <name>` selects the headless tool (headless still supports `claude` only — other tools are rejected up front pointing you at `coi shell --tool`). The flag is applied at tool-resolution time, so it overrides `[tool] name` from every config scope including a project `.coi/config.toml`, and the per-tool `[tool.<name>]` model/effort settings resolve correctly. Session history stays **per-tool** (`~/.coi/sessions-<tool>`), so `--resume`/`--continue` act on the chosen tool's own history — history is not shared across tools (their transcript formats are incompatible). An unknown name fails fast with the supported list. Covered by Go unit tests (override resolution, run headless rejection) and pytest.
