...
- [Bug Fix] **Network ACL cleanup warning on poweroff** - Fixed spurious "Warning: failed to delete network ACL: exit status 1" message when stopping containers with `poweroff` or `sudo shutdown 0`. The issue occurred because cleanup tried to delete ACLs while they were still attached to the container's network device. Now deletes the container first (which detaches the ACL), then cleans up the ACL without errors. Also added existence check to ACL deletion to gracefully handle already-removed ACLs. (#67)
