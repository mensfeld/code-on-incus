...
- [Feature] **Self-contained profile directories** — Profiles can now be defined as self-contained directories under `profiles/` (at system, user, or project level), each with its own `config.toml` and optional build script. Profile directory config mirrors the main config structure and supports all fields: `image`, `persistent`, `environment`, `forward_env`, `[tool]` (name, permission_mode, effort_level), `[build]` (base, script resolved relative to profile directory, commands), `[[mounts]]`, `[network]` (mode, allowed_domains), and `[limits]`. Directory structure: `.coi/profiles/NAME/config.toml`. Precedence: system dirs < system inline < user dirs < user inline < project dirs < project inline < CLI flags. Added `coi profile list` (tabular view with name, image, persistent, source) and `coi profile show <name>` (full TOML-like detail view). Invalid profile directories (missing config.toml, broken TOML) are silently skipped. Includes Go unit tests and Python integration tests with edge cases. Partial implementation of #114.
