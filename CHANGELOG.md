...
- **Session data no longer lost when container shuts down via `sudo poweroff`** — When a user ran `sudo poweroff` inside a container, COI's cleanup goroutine would immediately attempt to pull the tool config directory (`.claude`, `.config/opencode`, etc.) via SFTP while the container's processes were still mid-teardown. The SFTP subsystem would fail with `error receiving version packet from server: unexpected EOF` or `Failed getting instance SFTP connection: bad file descriptor`, causing the session save to be silently skipped — meaning `--resume` would fail to restore the previous session. The fix eliminates the race at the source: session data is now saved only after the container's running state is confirmed — either confirmed still-running (normal `exit` path, SFTP fully functional) or confirmed fully stopped (poweroff path, SFTP stable on stopped containers). SFTP is never called during the mid-shutdown window. Affects all tools: Claude Code, opencode, and any future tool with a config directory. (#397)
