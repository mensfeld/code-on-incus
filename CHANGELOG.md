...
- [Bug Fix] **Docker commands now work without sudo for the code user** - The `code` user was correctly added to the `docker` supplementary group, but `incus exec --group` may not call `initgroups()` to load supplementary groups depending on the Incus version/configuration, leaving the docker socket inaccessible without `sudo`. Fixed by configuring the Docker daemon to use `"group": "code"` in `/etc/docker/daemon.json`, making the socket owned by `root:code` (0660) instead of `root:docker`. The `code` user's primary group is always active regardless of how the session is started, so `docker` works without `sudo` in all session types. Adds a test that explicitly runs `docker run` as UID 1000 to catch regression. Fixes #134.
