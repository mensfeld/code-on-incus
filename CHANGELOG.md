...
- **Switch base image source from LXC CDN to Canonical's official Ubuntu remote** — `coi build` previously pulled `images:ubuntu/22.04` from the LXC community image server, which is distributed via a third-party CDN (mirrors like `sgp1lxdmirror01.do.letsbuildthe.cloud`). This CDN is blocked in some corporate networks. The base image is now pulled as `ubuntu:22.04` from Canonical's own infrastructure (`cloud-images.ubuntu.com`), which is the `ubuntu:` remote built into Incus by default. The resulting `coi-default` image is identical — only the download source changes. **If you are upgrading:** run `coi build --force` to rebuild the image from the new source. (#388)
