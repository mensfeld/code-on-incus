...
- **Profile-based base image override for all builds** — The `[container.build] base` config option now applies to the default `coi-default` image as well as custom images. Previously it was only respected for custom profile builds. Users whose network blocks the default `images:ubuntu/22.04` CDN can now set an alternative base in `~/.coi/config.toml` or `.coi/config.toml`. When a `ubuntu:` remote reference is used, COI automatically registers the `ubuntu:` simplestreams remote in Incus if not already present. (#388)
