...
// tmpMountIsBounded reports whether /proc/mounts shows /tmp as a tmpfs that
// already carries an explicit size= option — i.e. it is already size-limited.
// A /tmp that lives on the rootfs (non-tmpfs) is NOT treated as bounded here:
// coi still wants to cap it so a runaway can't fill the container. Kept pure so
// it is unit-testable. (#728)
func tmpMountIsBounded(procMounts string) bool {
...
func (m *Manager) SetTmpfsSizeIfUnbounded(size string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to read /proc/mounts to probe /tmp: %w", err)
	}
