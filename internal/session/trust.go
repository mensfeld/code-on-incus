...
func escapingUntrustedMounts(mounts []MountEntry, workspace string) []MountEntry {
	...
	for _, m := range mounts {
		if m.Untrusted && m.HostPath != "" && !isWithinWorkspace(workspace, m.HostPath) {
			out = append(out, m)
		}
	}
	return out
...
func MountFingerprint(mounts []MountEntry) string {
	...
	for _, m := range mounts {
		lines = append(lines, fmt.Sprintf("%s|%s|%t", m.HostPath, m.ContainerPath, m.Readonly))
	}
	sort.Strings(lines)
...
func FilterTrustedMounts(mc *MountConfig, workspace string) (*MountConfig, []MountEntry) {
	...
	for _, m := range mc.Mounts {
		if m.Untrusted && !isWithinWorkspace(workspace, m.HostPath) && !trustedSource[m.SourcePath] {
