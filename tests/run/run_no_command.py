...
Tests that:
1. Run coi run without a command in a workspace with no coi-run
...
def test_run_no_command(coi_binary, cleanup_containers, workspace_dir):
    """
    Test that coi run without a command and without coi-run shows an error.

    Flow:
    1. Run coi run (no command) in a workspace without coi-run
    ...
    assert "coi-run" in combined_output, (
        f"Error should mention the coi-run convention. Got:\n{combined_output}"
    )
