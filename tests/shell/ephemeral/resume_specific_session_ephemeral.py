...
def test_resume_specific_session(coi_binary, cleanup_containers, workspace_dir):
    ...
    for line in lines:
        if "Saved Sessions:" in line:
            ...
            continue
        if in_sessions_section and line.strip().startswith("coi-"):
            # Extract session ID (format: "  coi-xxx-1-timestamp")
            first_session_id = line.strip().split()[0]
            break

    assert first_session_id is not None, \
        f"Should find first session ID in output:\n{result.stdout}"
