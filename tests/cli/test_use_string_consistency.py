...
def test_tmux_send_help_uses_angle_brackets(coi_binary):
    ...
    assert "<session-name>" in combined, f"Expected '<session-name>' in help output, got: {combined}"
...
def test_info_help_uses_lowercase(coi_binary):
    ...
    assert "SESSION_ID" not in combined, f"Old 'SESSION_ID' style should not appear, got: {combined}"
